        ...
        <section class="header text-center">
            <div class="mx-auto w-full xl:max-w-7xl 2xl:max-w-[1640px] px-3 sm:px-8 md:px-12">
                ...
                <div class="flex flex-col items-center justify-center">
                    <h2 class="text-2xl font-semibold md:text-3xl p-2 md:p-5">공지사항 수정하기</h2>
...
export default {
    setup () {
        ...
        const isPrimary = ref(currentNotice.value.primary);        // 메인 등록 여부
        const title = ref(currentNotice.value.title);              // 제목
        const content = ref(currentNotice.value.content);          // 내용
        const condition = ref(currentNotice.value.condition);      // 중요도 ex. 긴급, 중요 etc
        const belong = ref(currentNotice.value.belong);            // 소속 ex. 전체, ~학교 etc
        const author = ref(currentNotice.value.author);            // 저자
        const tempImg = ref([]);         // 임시 이미지
        const img = ref([]);             // 이미지
        
        /* 이미지가 존재하는지 검사 */
        if(Object.keys(currentNotice.value.img).length == 0){
            console.log('빈 객체다!');
        } else {
            console.log('객체다!');
            tempImg.value = currentNotice.value.img.link;         // 임시 이미지
            img.value = currentNotice.value.img;             // 이미지
        }
        ...
        const onSubmit = async () => {
            ...
            const notice = {
                ...
                createdAt: currentNotice.value.createdAt, 
            }

            console.log('수정할 공지사항은?', notice);

            store.updateNotice(notice).then(() => {
                router.go(-1);
            })
        }
