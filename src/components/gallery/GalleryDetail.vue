    ...
    <div class="bg-black min-h-screen flex flex-col items-center justify-center">
        <div class="max-w-sm md:max-w-md 2xl:max-w-[1640px] m-auto">
            <div class="relative w-full p-5 md:p-10 min-h-screen md:h-full">
                <!-- Header -->
                <div class="absolute flex pt-10 md:pt-13 space-x-1 md:space-x-10 z-10">
                    <!-- Back Button -->
                    <button
                        @click="$router.go(-1)"
                        class="
                ...
                <!-- menu -->
                <div v-show="open" class="absolute bg-white right-6 rounded-md shadow-xl mt-16 md:mt-24 text-sm md:text-xl font-semibold md:font-bold z-10">
                    <ul class="md:space-y-2">
                        ...
                        <li @click="deleteGallery" class="block px-2 md:px-4 py-2 text-red-400">
...
import Thumbnail from "../common/Thumbnail.vue";
    ...
    export default {
        ...
        setup () {
            ...
            /* Delete Gallery */
            const deleteGallery = () => {
                const id = store.currentGallery.id;

                if (confirm("정말 삭제합니까?")) {

                    store.deleteGallery(id).then(() => {
                        router.go(-1);
                    });
                } else {
                    alert("취소되었습니다.");
