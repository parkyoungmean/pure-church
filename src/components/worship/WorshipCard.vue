<template>
    ...
    <div class="worship-card relative">
        ...
        <div v-show="path!=='/worships'" class="absolute bottom-0 flex items-center jsustify-center text-white text-xl md:text-lg xl:text-sm 3xl:text-[0.95rem]">
            <p class="w-36 md:w-25 xl:w-18 3xl:w-22 text-center font-extrabold p-1 md:p-1 xl:p-0.5" :class="[item.belong === '어린이예배' ? 'bg-[#f9d423]' : item.belong === '청소년예배' ? 'bg-[#09A6FF]' : item.belong ==='간증' ? 'bg-[#7978FF]' : item.belong ==='복음학교' ? 'bg-amber-200' : item.color]" >{{item.belong}}</p>
