<template>
  <div>
    <!-- Close Button -->
    <div class="flex justify-end p-3 md:p-10">
        <button
        @click="$router.go(-1)"
        class="
            text-blue-400
    ...
    <section class="text-center">
      <div class="mx-auto mzx-w-7xl px-6 sm:px-8 md:px-12">
        ...
        <p class="mx-auto my-3 max-w-xl text-sm">
          메인 홈페이지의 슬라이드 광고를 관리합니다.
        </p>
        ...
        >
          <div
            ...
            :key="index"
            class="grid2-item bg-orange-100"
          >
            <h1
              class="title absolute font-bold text-xl px-12 py-16"
              :class="`${publicity.color.titleTextColor}`"
            >
              {{ publicity.title }}
            </h1>
            <img
              :src="`${publicity.img.link}`"
            ...
            >
              ...
              <button
              @click="deletePublicity(publicity.id)"
                class="
                  inline-flex
                  justify-center
                  px-2
...
export default {
  setup() {
    ...
    const deletePublicity = (id) => {
      if (confirm("정말 삭제합니까?")) {
        /* alert("ok"); */

        store.deletePublicity(id).then(() => {
        });
      } else {
        alert("취소되었습니다.");
      }
